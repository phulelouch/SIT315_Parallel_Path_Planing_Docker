...
bool isCarTooClose(int const lane, json sensor_fusion, int const range_in_mts, double const car_s,
                   int const prev_size, bool const check_behind)
{
    int laneRange = 2 + 4 * lane;
#pragma omp parallel
    {
#pragma omp for
        for (auto sensor: sensor_fusion) {
            float d = sensor[6];
            if (d < laneRange + 2 && d > laneRange - 2) {
                double vx = sensor[3];
                double vy = sensor[4];
                double check_speed = sqrt(vx * vx + vy * vy);
                double check_car_s = sensor[5];

                check_car_s += (double) prev_size * .01 * check_speed;
                if ((check_car_s > car_s && check_car_s < range_in_mts + car_s) ||
                    (check_behind && check_car_s < car_s && car_s < range_in_mts / 4 + check_car_s)) {
                    return true;
                }
            }
        }
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // When previous state almost empty, use the car as starting point
                    if (prev_size < 2) {
                        ...
                        // Use previous path's point as a starting reference
                    ...
                    // In frenet add evenly 30m appart ahead of starting reference
...
#pragma omp for
                    ...
                    // Actual points we will use later on for planner
...
#pragma omp for
                        ...
                        for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
                            ...
                            double N = (target_dist / (.01 * ref_vel / 2.24));
